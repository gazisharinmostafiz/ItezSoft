...
@section('content')
    ...
    <div class="bg-white shadow-md rounded-lg p-6 md:p-8">
        <form action="{{ route('admin.pages.store') }}" method="POST" id="createPageForm">
            ...
            {{-- Slug (Optional - if you want to allow manual setting, otherwise model handles it) --}}
            {{--
            <div class="mb-6">
                <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">URL Slug (Optional - will be auto-generated if left blank)</label>
                <input type="text" name="slug" id="slug"
                       ...
                       value="{{ old('slug') }}" placeholder="e.g., about-our-company">
                @error('slug')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="hidden" name="content" id="content_output">
                @error('content')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="is_published" class="flex items-center text-sm font-medium text-gray-700">
                    <input type="hidden" name="is_published" value="0"> {{-- Default value if checkbox is not checked --}}
            ...
            <div class="mb-6">
                ...
                <textarea name="meta_description" id="meta_description" rows="3"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('meta_description') border-red-500 @enderror">{{ old('meta_description') }}</textarea>
            ...
            const quill = new Quill('#quill-editor', {
                ...
                modules: {
                    toolbar: [
                        ...
                        [{ 'font': [] }],
                        [{ 'align': [] }],
                        ['link', 'image', 'video'], // Note: image/video might need custom handlers for server uploads
                        ['clean']
                    ]
                },
