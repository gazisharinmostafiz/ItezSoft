{{-- File: resources/views/admin/menus/items/edit.blade.php --}}
@extends('layouts.admin')

@section('title', 'Edit Item in Menu: ' . $menu->name)

@section('content')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-800">Edit Item: <span class="text-indigo-600">{{ $menuItem->title }}</span></h1>
            <p class="text-sm text-gray-500">Menu: {{ $menu->name }}</p>
            <a href="{{ route('admin.menus.items.index', $menu->id) }}" class="text-sm text-indigo-600 hover:text-indigo-800 mt-1 inline-block">
    ...
    <div class="bg-white shadow-md rounded-lg p-6 md:p-8">
        <form action="{{ route('admin.menus.items.update', ['menu' => $menu->id, 'menu_item' => $menuItem->id]) }}" method="POST" id="editMenuItemForm">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" id="title"
                       ...
                       value="{{ old('title', $menuItem->title) }}" required placeholder="e.g., Home, About Us">
                @error('title')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"
                 x-data="{
                    selectedType: '{{ old('link_type', $menuItem->link_type) }}',
                    urlValue: '{{ old('link_type', $menuItem->link_type) === 'url' ? old('link_value', $menuItem->link_value) : ($menuItem->link_type === 'url' ? $menuItem->link_value : '#') }}',
                    routeValue: '{{ old('link_type', $menuItem->link_type) === 'route' ? old('link_value', $menuItem->link_value) : ($menuItem->link_type === 'route' ? $menuItem->link_value : '') }}',
                    pageSlugValue: '{{ old('link_type', $menuItem->link_type) === 'page_slug' ? old('link_value', $menuItem->link_value) : ($menuItem->link_type === 'page_slug' ? $menuItem->link_value : '') }}'
                 }">
                {{-- Link Type --}}
                <div>
                    ...
                    <select name="link_type" id="link_type"
                        ...
                        <option value="url" {{ (old('link_type', $menuItem->link_type) == 'url') ? 'selected' : '' }}>Absolute URL</option>
                        <option value="route" {{ (old('link_type', $menuItem->link_type) == 'route') ? 'selected' : '' }}>Named Route</option>
                        <option value="page_slug" {{ (old('link_type', $menuItem->link_type) == 'page_slug') ? 'selected' : '' }}>Dynamic Page (by Slug)</option>
                    </select>
                    @error('link_type')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div x-show="selectedType === 'url'" id="link_value_display_area_url">
                        <input type="url" id="link_value_url_display" x-model="urlValue"
                               ...
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @if($errors->has('link_value') && old('link_type', $menuItem->link_type) === 'url') border-red-500 @endif">
                    </div>
                    <div x-show="selectedType === 'route'" id="link_value_display_area_route">
                        <select id="link_value_route_display" x-model="routeValue"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @if($errors->has('link_value') && old('link_type', $menuItem->link_type) === 'route') border-red-500 @endif">
                            <option value="">Select a Route</option>
                            @foreach($namedRoutes as $routeName => $routeDisplay)
                                <option value="{{ $routeName }}" {{ (old('link_type', $menuItem->link_type) === 'route' && old('link_value', $menuItem->link_value) == $routeName) ? 'selected' : '' }}>{{ $routeDisplay }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div x-show="selectedType === 'page_slug'" id="link_value_display_area_page">
                        <select id="link_value_page_slug_display" x-model="pageSlugValue"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @if($errors->has('link_value') && old('link_type', $menuItem->link_type) === 'page_slug') border-red-500 @endif">
                            <option value="">Select a Page</option>
                            @foreach($pages as $page)
                                <option value="{{ $page->slug }}" {{ (old('link_type', $menuItem->link_type) === 'page_slug' && old('link_value', $menuItem->link_value) == $page->slug) ? 'selected' : '' }}>{{ $page->title }} (/{{ $page->slug }})</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="parent_id" id="parent_id"
                        ...
                        @foreach ($parentItems as $item) {{-- $parentItems should exclude current item and its children --}}
                            <option value="{{ $item->id }}" {{ old('parent_id', $menuItem->parent_id) == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                        @endforeach
                    </select>
                     <p class="mt-1 text-xs text-gray-500">Select a parent to make this a sub-item. Cannot be self or own child.</p>
                    @error('parent_id')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" min="0"
                           ...
                           value="{{ old('order', $menuItem->order) }}" required>
                    @error('order')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="target" id="target"
                        ...
                        <option value="_self" {{ old('target', $menuItem->target) == '_self' ? 'selected' : '' }}>Same Window/Tab (_self)</option>
                        <option value="_blank" {{ old('target', $menuItem->target) == '_blank' ? 'selected' : '' }}>New Window/Tab (_blank)</option>
                    </select>
                    @error('target')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="icon_class" id="icon_class"
                           ...
                           value="{{ old('icon_class', $menuItem->icon_class) }}" placeholder="e.g., fas fa-home">
                    @error('icon_class')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
