...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Edit User: <span class="font-normal">{{ $user->name }}</span></h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6 md:p-8">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="mb-6">
                <label for="password" class="block text-sm font-medium text-gray-700 mb-1">New Password (Optional - leave blank to keep current password)</label>
            ...
            <div class="mb-6">
                <label for="is_admin" class="flex items-center text-sm font-medium text-gray-700">
                    <input type="hidden" name="is_admin" value="0"> {{-- Default value if checkbox is not checked --}}
                    <input type="checkbox" name="is_admin" id="is_admin" value="1"
                           class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                           {{ old('is_admin', $user->is_admin) ? 'checked' : '' }}
                           @if(auth()->id() === $user->id && $user->is_admin && \App\Models\User::where('is_admin', true)->count() <= 1)
                               disabled
                               title="Cannot remove admin status from the only admin account."
                           @endif
                           >
                    <span class="ml-2">Assign as Administrator</span>
                </label>
                @if(auth()->id() === $user->id && $user->is_admin && \App\Models\User::where('is_admin', true)->count() <= 1)
                    <p class="mt-1 text-xs text-yellow-600">You cannot remove your own admin status if you are the only administrator.</p>
                @endif
                @error('is_admin')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
